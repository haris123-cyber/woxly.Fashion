...
export function CartDrawer() {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent className="flex flex-col w-full sm:max-w-md bg-background border-l border-border text-foreground p-6">
        <SheetHeader className="mb-6">
          <SheetTitle className="text-left font-fraunces text-2xl font-normal text-foreground">Your Cart ({itemCount})</SheetTitle>
        </SheetHeader>
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto -mx-6 px-6 space-y-8 hide-scrollbar">
              {!qualified && (
                <div className="space-y-3">
                  ...
                  <div className="h-1 rounded-none bg-muted overflow-hidden">
                    <div
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={`${item.productId}-${variantKey}`} className="flex gap-4">
                      <div className="relative h-28 w-20 bg-secondary border border-border shrink-0">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          <Link
                            ...
                            className="font-fraunces text-lg text-foreground hover:text-[#cfae70] transition-colors line-clamp-1"
                          >
                            {item.name}
                          </Link>
                          {item.variant && (
                        ...
                        <div className="flex items-center gap-3 mt-4">
                          <button
                            className="h-8 w-8 border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-[#cfae70] transition-colors"
                          ...
                          <button
                            className="h-8 w-8 ml-auto text-muted-foreground hover:text-red-500 transition-colors flex items-center justify-center"
            ...
            <div className="border-t border-border pt-6 space-y-6 mt-6">
              <div className="flex justify-between items-end">
                <span className="text-[10px] tracking-[0.2em] uppercase text-muted-foreground">Subtotal</span>
              ...
              <div className="space-y-3">
                ...
                <Button asChild variant="outline" className="w-full border-border hover:border-[#cfae70] text-muted-foreground hover:text-foreground rounded-none uppercase font-bold text-[10px] tracking-[0.2em] h-12 bg-transparent transition-colors" onClick={() => setOpen(false)}>
